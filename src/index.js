		...
		connection.onLogs(
			...
			async ({ logs, err, signature }) => {
				try {
					...
					/** You need to use an RPC provider for getparsedtransaction to work properly.
					...
					if (parsedTransaction && parsedTransaction?.meta.err == null) {
						...
						logger.info(`Attempting to buy token: ${poolKeys.baseMint.toString()} with ${BUY_AMOUNT_SOL} SOL...`);
						
						const buyResult = await buyToken({
							connection,
							wallet: sniperWallet,
							poolKeys,
							amountInSol: BUY_AMOUNT_SOL,
							logger,
						});

						if (buyResult && buyResult.success) {
							logger.info(`Successfully bought token! Signature: ${buyResult.signature}`);
							seenSignatures.add(signature);
							
							// Store purchased token data
							storeData(newDataPath, {
								mint: poolKeys.baseMint.toString(),
								signature: buyResult.signature,
								timestamp: new Date().toISOString(),
							});
						} else {
							logger.error(`Failed to buy token: ${poolKeys.baseMint.toString()}`);
						}
					}
				} catch (error: any) {
					logger.error(`Error processing log/transaction for signature ${signature}: ${error.message}`);
				}
			},
			"confirmed"
		);
	} catch (error: any) {
		logger.error(`Error in monitorNewTokens: ${error.message}`);
		// Retry on disconnection
		setTimeout(() => monitorNewTokens(connection, sniperWallet), 5000);
	}
}
